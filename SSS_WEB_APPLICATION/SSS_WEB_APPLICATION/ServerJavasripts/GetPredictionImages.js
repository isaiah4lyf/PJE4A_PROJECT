...
function GET_LOGIN_RESULT(results) {
    PREDICTION_IMAGES(results["Id"], results["User_Name"]);
}
function PREDICTION_IMAGES(id, username) {
    SSS_WEB_APPLICATION.SSS_SERVICE_LOCAL.RETURN_PREDICTION_IMAGES(id, username, GET_DATA);
}
function GET_DATA(results) {
    ...
    for (index = 0; index < results.length; ++index) {
        ...
        //images +=    "<div>";
        //images +=        "Prediction Correct: " + results[index]["Prediction_Correct"];
        //images +=    "</div>";
        //images += "</div>";
    }
...
function UPDATE_IMAGES() {
    SSS_WEB_APPLICATION.SSS_SERVICE_LOCAL.RETURN_PREDICTION_IMAGES("20", "isaiah", GET_DATA_FOR_UPDATE);
}
function GET_DATA_FOR_UPDATE(results) {
    if (InitialArray.length < results.length) {
        ...
        //images += " <div>";
       //images += "     Prediction Correct: " + results[results.length - 1]["Prediction_Correct"];
       // images += " </div>";
        //images += "</div>";
        document.getElementById("Images").innerHTML += images;
        InitialArray = results;
